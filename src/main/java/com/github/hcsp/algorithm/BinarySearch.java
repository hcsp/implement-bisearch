...
public class BinarySearch {
    public static void main(String[] args) {
        ...
        System.out.println(binarySearchByRecursion(new String[]{"aaa", "ccc", "fff", "yyy", "zzz"}, "yyy", 0, 4));
    }
    ...
    //递归
    private static int binarySearchByRecursion(String[] strings, String target, int start, int end) {
        while (start <= end) {
            int mid = start + ((end - start) >>> 1);
            if (strings[mid].compareTo(target) < 0) {
